#!/usr/bin/perl -w
#####
# TJN: (12may2016) A 'hacked' version of Kevin's build.pl script to work
...
# XXX: Error if we try to build Pisces
if ($program_args{build_pisces}) {
    # XXX: Hack to avoid pisces for now.
    print "ERROR: Currently disabled support for building Pisces.\n";
    exit (1);
}
...
#
    ...
    # If exists, copy over password file
    if (-f "$BASEDIR/passwd.cubswin" ) {
        #-----
        # XXX: OVERRIDE DEFAULT PASSWORD! 
        #      Must be after above OVERLAYDIR/skel rsync.
        #-----
        system("cp -f $BASEDIR/passwd.cubswin $IMAGEDIR/etc/passwd") == 0
			or die "Failed to copy passwd.cubswin to $IMAGEDIR";
	}

	# Install numactl into image
	if ( -d "$SRCDIR/$numactl{basename}/_install" ) {
		system("rsync -a $SRCDIR/$numactl{basename}/_install/\* $IMAGEDIR/") == 0
			or die "Failed to rsync numactl to $IMAGEDIR";
	} else {
		print "Skipping numactl - not installed in image.\n";
	}

	# Install hwloc into image
	if ( -d "$SRCDIR/$hwloc{basename}/_install" ) {
		system("rsync -a $SRCDIR/$hwloc{basename}/_install/\* $IMAGEDIR/") == 0
			or die "Failed to rsync hwloc to $IMAGEDIR";
	} else {
		print "Skipping hwloc - not installed in image.\n";
	}

 	# Install libhugetlbfs into image
	if ( -d "$SRCDIR/$libhugetlbfs{basename}/_install" ) {
		system("rsync -a $SRCDIR/$libhugetlbfs{basename}/_install/\* $IMAGEDIR/") == 0
			or die "Failed to rsync libhugetlbfs to $IMAGEDIR";
	} else {
		print "Skipping libhugetlbfs - not installed in image.\n";
	}

	# Install OpenMPI into image
	mkdir "$IMAGEDIR/opt/simple_busybox";
	if (-d "/opt/simple_busybox/$ompi{basename}" ) {
		system("cp -R /opt/simple_busybox/$ompi{basename} $IMAGEDIR/opt/simple_busybox") == 0
			or die "Failed to rsync OpenMPI to $IMAGEDIR";
	} else {
		print "Skipping OpenMPI - not installed in image.\n";
	}
	

	if (-f "$SRCDIR/pisces/pisces/pisces.ko") { 
       ...
       system("cp -R $SRCDIR/test/null/null $IMAGEDIR/opt/hobbes") == 0
	    ...
	    print "Skipping Hobbes/Pisces - not installed in image.\n";
    }
	...
	if (-f "$SRCDIR/dtk/BUILD/DataTransferKit/packages/Adapters/STKMesh/example/DataTransferKitSTKMeshAdapters_STKInlineInterpolation.exe") {
	    ...
	    print "Skipping DTK - not installed in image.\n";
    }
